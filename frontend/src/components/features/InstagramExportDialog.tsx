...
import {
  ...
  CARD_H,
  ensureFont,
} from "../../utils/instagramCards/canvasUtils";
...
function buildCaption(edition: Edition, stories: Story[]): string {
  ...
  const hashtags = topicIds
    .map((id) => TOPIC_HASHTAGS[id])
    .filter(Boolean)
    .join(" ");
...
export default function InstagramExportDialog({
  open,
  onClose,
  edition,
  stories,
        ...
        <Box sx={{ mb: 4 }}>
          ...
          <Box
            sx={{
              ...
              overflow: "hidden",
              boxShadow:
                "0 0 40px rgba(99,102,241,0.15), 0 4px 20px rgba(0,0,0,0.3)",
              border: "1px solid",
              borderColor: "divider",
            }}
          >
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (canvas)
                    downloadCardAsPng(
                      canvas,
                      `edition-${edition.number}-story-${i + 1}`,
                    );
                }}
                sx={{ textTransform: "none" }}
              >
