...
const BASE_TAGS = ['#AI', '#AINews', '#up2daite', '#ArtificialIntelligence', '#Tech', '#TechNews']

const CTA = '📲 More at up2daite.com'
...
function editionCaption(e, editionStories) {
  ...
  const topicTags = editionStories.flatMap(s => (s.topics || []).map(t => tag(t.replace(/^ai-/, ''))))
  const hashtags = [...BASE_TAGS, ...topicTags, '#AIUpdate', '#Newsletter', '#WeeklyAI', '#AInews']
...
function modelCaption(m) {
  ...
  const hashtags = [
    ...
    tag(m.name),
    tag(m.category),
    '#LLM', '#AIModels', '#MachineLearning', '#GenAI',
  ]
  return { hook, body, hashtags }
}

function jobCaption(j) {
  const trendLabel = { rising: 'rising ↗️', stable: 'stable →', falling: 'falling ↘️' }[j.trend] || j.trend
  ...
  const hashtags = [
    ...BASE_TAGS,
    tag(j.category),
    '#FutureOfWork', '#Automation', '#JobsOfTheFuture', '#AIatWork',
  ]
  return { hook, body, hashtags }
}
...
async function main() {
  ...
  console.log(`✓ ${editions.length} editions · ${stories.length} stories · ${models.length} models · ${jobs.length} jobs`)
  ...
  const write = (dir, filename, caption) => {
    fs.writeFileSync(path.join(OUTPUT_DIR, dir, filename), renderCaption(caption))
  ...
  for (const e of editions) {
    ...
    write('editions', `edition-${e.number}-${slug(e.title)}.txt`, editionCaption(e, editionStories))
  }
